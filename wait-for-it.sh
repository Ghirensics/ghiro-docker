...
wait_for_wrapper() {
    ...
    if [[ $QUIET -eq 1 ]]; then
        timeout $BUSYTIMEFLAG $TIMEOUT $0 $wait_host:$wait_port --quiet --child --timeout=$TIMEOUT &
    else
        timeout $BUSYTIMEFLAG $TIMEOUT $0 $wait_host:$wait_port --child --timeout=$TIMEOUT &
    fi
    PID=$!
